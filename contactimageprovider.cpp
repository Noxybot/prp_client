...
Q_INVOKABLE ContactImageProvider::~ContactImageProvider()
{
    qDebug() << "ContactImageProvider DELETED";
}

ContactImageProvider::ContactImageProvider(QString server_ip, std::shared_ptr<SqlContactModel> sql_contact_model)
...
void ContactImageProvider::setCurrentUserLogin(QString login)
{
    if (login.length() == 0)
    {
...
QPixmap ContactImageProvider::requestPixmap(const QString &login, QSize *size, const QSize &requestedSize)
{
    Q_UNUSED(size);
...
void ContactImageProvider::error(QNetworkReply::NetworkError code)
{
    qDebug() << "error, QNetworkReply: " << code;
}

void ContactImageProvider::userImageFinished()
{
